...
class PackfileTask(Task):
    def __init__(self, uploader, audit_log, archive_fs, packfile_type, deid_profile, follow_symlinks, container, filename, paths=None, zip_path_transform=None, compression=None, max_spool=None):
    ...
    def execute(self):
        ...
        zip_member_count = create_zip_packfile(tmpfile, self.archive_fs, packfile_type=self.packfile_type,
            symlinks=self.follow_symlinks, paths=self.paths, zip_path_transform=self.zip_path_transform, compression=self.compression,
...
class UploadQueue(WorkQueue):
    ...
    def upload_packfile(self, archive_fs, packfile_type, deid_profile, container, filename, paths=None, zip_path_transform=None):
        if self.skip_existing and self.uploader.file_exists(container, filename):
        ...
        self.enqueue(PackfileTask(self.uploader, self.audit_log, archive_fs, packfile_type,
            deid_profile, self.follow_symlinks, container, filename, paths=paths, zip_path_transform=zip_path_transform,
